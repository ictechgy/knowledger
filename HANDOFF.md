...
3. 선택 도입/확장: 실제 SSO/KMS·모델 공급자/egress, SaaS connector·벡터 검색·파일럿.
   ...
   10만 문서 측정에서 오프셋 페이지네이션 O(N²) 결함(캐시 상한 초과 결과의 영구 거부)을
   찾아 `perf/browse-pagination-scale`에서 수정했다 — 브라우즈 개정본 캐시와 검색 매치
   캐시 모두 상한 초과 결과를 단일 대형 항목으로 유지한다. 혼합 슬롯·Fabric 어댑터
   측정 도구(`tools/performance-fabric.ts`)도 같은 브랜치에 있다.
