...
현재 공개 버전은 `v0.2.0`이다. 이후 main의 signing gateway, SSO 경계,
vector read model, model egress, 운영 드릴, Git 커넥터·파일럿 측정은 다음
minor 릴리스 후보 묶음이다. 서명 정책과 모델 egress의 호환성 변경은
`Unreleased`에 기록돼 있다. 후보 검증과 실제 게시 상태를 구분한다.
