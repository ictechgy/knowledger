...
저널은 읽기 전용으로 열어 채널과 해시 체인을 검증한다 — 파일이나 WAL sidecar를 만들지
않고, 빈·잘못된 파일은 측정으로 통과하지 않는다. 출력은 `schema_version: 1`의 측정
...
파일럿 전용 채널이나 기간이 제한된 저널을 준비하고, `window` 필드로 실제 집계 범위를
확인한다. 관찰 로그는 사람이 기록한 감사 입력이며 정규 파일·크기 제한으로 읽는다.
