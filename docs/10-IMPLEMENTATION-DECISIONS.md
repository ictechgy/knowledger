...
## 수신자별 자동 기한 알림 — 2026-09-20

앱 실행 중 현재 일정 담당자에게만 도래/초과 단계별 알림을 생성한다. 댓글/승인 이벤트로
위장하지 않고 private DB의 별도 테이블에 exact revision·schedule version·recipient·phase
복합 키로 보관한다. DB 임계 영역에서 일정을 재확인하고 중복을 제거한다. 일정 변경/완료 후
오래된 알림은 조회·unread에서 제외하며 읽음 상태와 중복 제거 기록은 복원 후에도 유지한다.
외부 전송은 없고 serving/membership은 생성 시, 현재 계정 인가는 표시 시 확인한다.
유한 배치·keyset 진행과 중단 시 늦은 쓰기 차단을 적용하며 지식 사용 판정은 바꾸지 않는다.
기본값·운영 한계는 [자동 기한 알림](34-REVIEW-REMINDERS.md)에 기록한다.

## 선택적 댓글 전달과 수신자 전용 보관 — 2026-09-20

앱 내부 댓글을 자동 전파하지 않고 작성자가 지정한 한 댓글·한 수신자만 고정 adapter로
전달한다. 수신 원문은 해당 사람의 inbox에 보관하고 로컬 공통 thread나 승인으로 편입하지
않는다. 기본 비활성·런타임 주입 설정이며 actor별 멱등 outbox/inbox는 기존 private DB와
백업 계약을 따른다. worker는 현재 원장/계정/선택 전송 정책을 재확인한 뒤 한정된 횟수로
시도한다. 전달 ID·내용 digest가 일치하는 수신 저장 영수증만 완료 근거로 사용한다.

HTTP adapter는 고정 HTTPS·redirect 거절·HMAC source/조직 바인딩을 사용하며 시험용
loopback HTTP만 명시 옵션으로 허용한다. HMAC은 발신 앱의 진술을 확인하며 개별 사람의
독립 서명이나 Fabric VALID 증명이 아니다. 시도 임대와 수신 중복 제거로 복구 가능한
at-least-once 전송을 구현하고, 목표 변경·권한 거절 시 원래 내용을 다른 대상으로 재해석하지
않는다. 상세 계약·가상 데모·실환경 한계는 [전달 안내](33-REVIEW-DELIVERY.md).

## 앱 단위 검토 협업과 영향 설명 — 2026-09-20

댓글·멘션·검토 일정·완료는 원장 합의와 별개의 운영 기록으로 둔다. 정확한 shared digest에
연결하고 기존 private DB의 별도 테이블에 이벤트·수신 알림·일정 CAS를 원자적으로 저장한다.
같은 앱의 현재 구성원만 접근한다. 조직별 Fabric 앱 간 복제나 외부 알림 전송은 구현하지 않는다.
사람만 일정을 설정/완료하고, 완료·기한 초과로 승인 또는 use 판정을 바꾸지 않는다.
기존 DB/스냅샷을 보존하는 additive schema이며 protocol/chaincode 계약은 바뀌지 않는다.

역방향 의존 조회는 검증된 canonical 개정과 전체 slot 일치에서 재계산한다. 정보성 경로를
필수 사용 조건으로 승격하지 않는다. 페이지는 정확한 checkpoint에 결속하고 현재 인가를
다시 확인한다. 규모 상한에서는 일부 목록을 완전한 것으로 반환하지 않고 명시적 오류를 낸다.
상세 [검토 워크스페이스](31-REVIEW-WORKSPACE.md). 런타임 조직/정책 변경은 초기 authority
binding을 풀어 구현하지 않으며 [별도 migration 설계](32-GOVERNANCE-EVOLUTION.md)에 남긴다.
...
강제하려는 배포는 `modelEgress.require_adapter`(어댑터 미지정 resolve를
...
### 비공개 초안의 의존관계 작성 — 2026-09-20

새 문서에 의존관계를 넣기 위해 domain fixture를 만들어야 했던 흐름을 draft API·UI로
연결한다. 입력은 공유 revision digest·relationship·enforcement로 한정하고 서버가
검증된 canonical 개정에서 전체 slot을 채운다. private source 식별자나 클라이언트가
주장하는 scope는 의존관계 입력으로 받지 않는다. 게시 전/철회된 공유 개정도 초안에는
선택할 수 있으며 실제 사용은 기존 활성 합의·의존성·fence·재검증 계약이 결정한다.

생성·수정·Markdown import에서 필드 생략은 기존 참조를 유지하고 명시적 배열은
전체를 교체한다. 참조만 바뀌어도 새로운 digest의 초안이 생기고 edit/import 멱등 입력에
결속된다. 기존 원본·shared parent·slot은 보존하며 이전 승인은 새 개정에 재사용되지 않는다.
브라우저는 개정 선택을 고정하고 비동기 base/참조 로딩 실패 시 저장을 보류한다.
조회 결과는 선택 후보이며 UI 캐시가 지식 사용 권한을 만들지 않는다.
