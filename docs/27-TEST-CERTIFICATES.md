...
2026-09-20 공개 인증서 점검은 **81개 모두 `ok`**였다. User1 세 인증서의
현재 만료일은 `2027-01-14T14:41:57Z`다. 개인키 읽기·갱신·앱 재시작 없이
공개 인증서의 유효기간만 확인한 결과이며, 네트워크 readiness 검증은 아니다.

14일 경고 창은 **2026-12-31T14:41:57Z (2026-12-31 23:41:57 KST)**부터다.
다음 수동 점검일은 **2027-01-01 KST**로 둔다. 저장소 관리자가 위 `check`를
실행하고 `expiring`이면 유지보수 창에서 기존 `prepare`/`apply`/재시작 절차를
수행한다. 날짜를 기록한 것이며 자동 예약·갱신 작업은 설치하지 않았다.
인증서가 이후 교체되면 실제 `valid_to`를 기준으로 다음 일정을 다시 계산한다.
