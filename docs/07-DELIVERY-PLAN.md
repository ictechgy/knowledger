...
공통 합의 엔진, API/UI·resolver/manifest, 실제 로컬 Fabric 네트워크와 영속 full-block projection까지 개발 알파로 구현했다. OIDC·서명·권한 회수, 조직별 개발 실행, Markdown 가져오기·비공개 초안 재개, 조직 scope를 보존하는 DB 복원도 연결했다. [실행 가이드](11-RUNTIME.md)가 실제 지원 범위다. 아래 P0–P3의 운영 통합 전체가 완료된 것은 아니다. 실제 회사 SSO/KMS, 독립 조직·호스트 장애, 전체 인프라 재해 복구와 성능 시험은 남아 있다. 요청 추적·개정본 비교·브라우저 회귀·성능 및 복원력 시험 자동화, Markdown 저장소 adapter와 guarded-generation 클라이언트도 구현했다. P4의 실제 사용 효과 비교·운영 대시보드·외부 공급자 배포 전체를 완료한 것은 아니다. 실행한 테스트와 실제 네트워크 검증은 [검증 기록](VALIDATION.md)에 분리했다. 웹 디자인의 후속 방향은 [DESIGN.md](../DESIGN.md)에 있다.
