...
`npm run test:backup`은 같은 절차를 CI에서 반복하는 리허설이다 — 새 런타임을 만들고 정상 종료한 뒤
오프라인 스냅샷→새 디렉터리 복원→체크포인트·저널·비공개 초안 일치를 확인하고, WAL 잔존·기존 대상·
중첩 경로 거부 가드도 함께 검사한다. 세부는 [자동 실험](22-AUTOMATED-EXPERIMENTS.md)에 있다.
