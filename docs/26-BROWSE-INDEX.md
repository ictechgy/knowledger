...
일반 풀의 상한은 **8개 결과 집합, 총16,384 refs, 추정512KiB**다.
refs 수가 일반 상한을 넘는 선택 집합은 별도 대형 풀에 **최대8개, 합산 추정16MiB**로
보관한다. 각 풀은 자기 예산 안에서 LRU로 축출하며 서로를 축출하지 않는다.
참조 포인터 배열만 보관하고 본문을 복제하지 않는다. 추정 바이트는 JS heap의 실측 상한은 아니다.
...
제한하고 본문·원래 검색어를 보관하지 않는다. 20,000 IDs를 넘는 결과는 별도 대형 풀에
**최대8개, UTF-8 key/ID 합산64MiB**로 보관한다. 빈 결과도 일반 풀에 캐시한다.
건수는 작지만 key/ID 바이트만 일반 예산을 넘는 결과나 대형 풀 전체 예산보다 큰 결과는
캐시하지 않고 다시 계산한다. UTF-8 바이트 예산과 실제 JS heap 사용량은 다르다.
...
현재 참조는 entity 수에 비례해 유지한다. 인덱스 쓰기는 검증된 delta를 준비하고 커밋하며,
블록마다 전체 Map을 복사하지 않는다. 정렬 배열 병합·삽입 비용은 변경과 기존 참조 수의
영향을 받는다. Fabric의 durable ingest도 `prepareBlock`의 검증된 쓰기분만 SQL 커밋 뒤
반영하므로 매 블록 `fork()` 전체 상태 복사를 하지 않는다. 명시적 `fork()` 호출은 여전히
O(상태) 얕은 복사다. 준비한 블록은 한 번만 커밋할 수 있고 다른 블록이 먼저 커밋되면 거부한다.

두 대형 질의가 예산 안에 들어오면 교대로 페이지를 읽어도 서로를 축출하지 않는다.
하지만 동시 작업 세트가 각 풀의 8개/합산 바이트 예산을 넘으면 다시 계산한다.
캐시가 없는 snapshot 집합 계산은 메타데이터를 순회하고 새 substring 검색은 후보 원문을
읽는다. 전체 이력 검증, 현재 상태 O(keys), Fabric의 cold snapshot replay 비용은 남는다.
측정은 [검증 기록](VALIDATION.md)에 분리하며, 캐시 상한이나 합성 측정을 운영 SLA로 삼지 않는다.
