...
test('incomplete authentication flags fail before any unauthenticated runtime is created', t => {
  const directory = mkdtempSync(join(tmpdir(), 'knowledger-auth-cli-'));
...
test('organization CLI flags cannot enable an unauthenticated or local runtime', t => {
  const directory = mkdtempSync(join(tmpdir(), 'knowledger-organization-cli-'));
  t.after(() => rmSync(directory, {recursive:true,force:true}));
...
test('product startup requires configuration and never creates an implicit demo',t=>{
  const directory=mkdtempSync(join(tmpdir(),'knowledger-no-default-'));t.after(()=>rmSync(directory,{recursive:true,force:true}));
...
test('config initializer supports four organizations and refuses to overwrite a file',t=>{
  const directory=mkdtempSync(join(tmpdir(),'knowledger-init-cli-'));t.after(()=>rmSync(directory,{recursive:true,force:true}));
