...
function overwriteBaselineMetrics(path: string, value: number): void {
  const data = JSON.parse(readFileSync(path, 'utf8'));
  for (const name of Object.keys(data.metrics)) {
    const metric = data.metrics[name];
    data.metrics[name] = metric && typeof metric === 'object' ? { ...metric, p95_ms: value } : value;
  }
...
test('performance smoke CLI accepts a non-regressed rerun against a baseline', async t => {
  ...
  const baseline = runSmokeCli(root, [...flags, '--out', 'baseline.json']);
  assert.equal(baseline.status, 0, baseline.stderr);
  // 실제 타이밍 지터에 의존하지 않도록 baseline 메트릭을 큰 값으로 덮어써 비회귀를 결정적으로 만든다.
  overwriteBaselineMetrics(join(root, 'baseline.json'), 1e12);
  const rerun = runSmokeCli(root, [...flags, '--baseline', 'baseline.json', '--threshold', 'search=10,overview=10,publish=10,search_warm=10,search_cold=10,replay_restart_ms=10', '--out', 'comparison.json']);
...
test('performance smoke CLI keeps the result file and exits nonzero on threshold regression', async t => {
  ...
  overwriteBaselineMetrics(join(root, 'baseline.json'), 0);
  const regression = runSmokeCli(root, [...flags, '--baseline', 'baseline.json', '--threshold', 'publish=0,search=0,search_warm=0,search_cold=0,overview=0,replay_restart_ms=0', '--out', 'comparison.json']);
  ...
  assert.ok(preserved.comparison.regressions.length >= 1);
...
test('performance smoke CLI rejects incomparable baselines with actionable errors', async t => {
  ...
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const baseline = runSmokeCli(root, ['--documents', '2', '--samples', '1', '--body-bytes', '1', '--out', 'baseline.json']);
  assert.equal(baseline.status, 0, baseline.stderr);
  const mismatched = runSmokeCli(root, ['--documents', '3', '--samples', '1', '--body-bytes', '1', '--baseline', 'baseline.json']);
  assert.equal(mismatched.status, 1);
  assert.match(mismatched.stderr, /not comparable/);
  assert.match(mismatched.stderr, /dataset\.documents_requested/);
  const missing = runSmokeCli(root, ['--documents', '2', '--baseline', 'missing.json']);
  assert.equal(missing.status, 1);
  assert.match(missing.stderr, /Create one first/);
  const badThreshold = runSmokeCli(root, ['--documents', '2', '--baseline', 'baseline.json', '--threshold', 'unknown=0.1']);
  assert.equal(badThreshold.status, 1);
  assert.match(badThreshold.stderr, /unknown threshold metric/);
  const duplicateThreshold = runSmokeCli(root, ['--documents', '2', '--baseline', 'baseline.json', '--threshold', 'search=0.1,search=0.2']);
  assert.equal(duplicateThreshold.status, 1);
  assert.match(duplicateThreshold.stderr, /duplicate threshold metric/);
  const emptyRatio = runSmokeCli(root, ['--documents', '2', '--baseline', 'baseline.json', '--threshold', 'search= ']);
  assert.equal(emptyRatio.status, 1);
  assert.match(emptyRatio.stderr, /not a finite nonnegative ratio/);
  const thresholdWithoutBaseline = runSmokeCli(root, ['--documents', '2', '--threshold', 'search=0.1']);
  ...
  const sameFile = runSmokeCli(root, ['--documents', '2', '--samples', '1', '--body-bytes', '1', '--baseline', 'baseline.json', '--out', 'baseline.json']);
...
test('parseThresholds rejects malformed pairs without spawning the CLI', async () => {
  ...
  for (const text of ['search', '=0.1', 'search=', 'search= ', 'search=-0.5', 'search=abc', 'unknown=0.1', 'search=0.1,search=0.2']) {
    assert.throws(() => parseThresholds(text, allowed), ComparisonInputError, text);
  }
});
...
test('reportCliResult preserves the measured result when comparison input fails', async () => {
  ...
  try {
    ...
    assert.equal(written.comparison.baseline, 'baseline.json');
    assert.match(written.comparison.error, /not comparable/);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test('performance-fabric CLI compares against a baseline when optional deps are installed', async t => {
  ...
  overwriteBaselineMetrics(join(root, 'baseline.json'), 1e12);
  const rerun = fabricCli([...flags, '--baseline', 'baseline.json', '--threshold', 'ingest_total_ms=10,search=10,overview=10,replay_restart_ms=10', '--out', 'comparison.json']);
  assert.equal(rerun.status, 0, rerun.stderr);
  const compared = JSON.parse(rerun.stdout);
  assert.equal(compared.mode, 'fabric-adapter-synthetic');
  assert.deepEqual(compared.comparison.regressions, []);
  assert.match(compared.dataset.journal_digest, /^[0-9a-f]{64}$/);
  assert.equal(compared.dataset.journal_source, 'generated');
});
