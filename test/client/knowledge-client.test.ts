...
import { KclClient, KclClientError, KclPendingError } from '../../packages/client/knowledge-client.ts';
...
test('client pins an origin, injects caller headers, rejects redirects, and prefixes only workspace paths', async () => {
  const calls: any[] = [];
  const client = new KclClient({ baseUrl: 'https://knowledge.example/', workspaceId: 'workspace-main', headers: async () => ({ Cookie: 'caller-owned', 'X-KCL-CSRF': 'csrf-owned' }), fetch: async (input, init) => { calls.push({ input: String(input), init }); return new Response('{}'); } });
  ...
  assert.throws(() => new KclClient({ baseUrl: 'http://example.com', workspaceId: 'workspace-main' }), /origin|HTTPS|loopback/i);
  assert.throws(() => new KclClient({ baseUrl: 'https://user:pass@knowledge.example', workspaceId: 'workspace-main' }), /origin|credential/i);
  await assert.rejects(() => client.request('https://other.example/escape'), KclClientError);
  await assert.rejects(() => client.request('/v1/workspaces/other/resolve'), (error: unknown) => error instanceof KclClientError && error.code === 'INVALID_PATH');
  await assert.rejects(() => client.request('/resolve/%2e%2e/escape'), (error: unknown) => error instanceof KclClientError && error.code === 'INVALID_PATH');
});

test('client bounds responses, parses strict JSON, and exposes 202 as a non-committed error', async () => {
  const pending = new KclClient({ baseUrl: 'https://knowledge.example', workspaceId: 'workspace-main', fetch: async () => new Response(JSON.stringify({ status: 'pending' }), { status: 202 }) });
  await assert.rejects(() => pending.request('/resolve', { method: 'POST', body: {} }), (error: unknown) => error instanceof KclPendingError && error.status === 202);
  const duplicate = new KclClient({ baseUrl: 'https://knowledge.example', workspaceId: 'workspace-main', fetch: async () => new Response('{"a":1,"a":2}') });
  await assert.rejects(() => duplicate.request('/resolve'), KclClientError);
  const huge = new KclClient({ baseUrl: 'https://knowledge.example', workspaceId: 'workspace-main', fetch: async () => new Response('x'.repeat(2 * 1024 * 1024 + 1)) });
  await assert.rejects(() => huge.request('/resolve'), (error: unknown) => error instanceof KclClientError && error.code === 'RESPONSE_TOO_LARGE');
  const stalled = new KclClient({ baseUrl: 'https://knowledge.example', workspaceId: 'workspace-main', timeoutMs: 10, fetch: async () => new Response(new ReadableStream({ start() { /* deliberately never produce a body */ } })) });
  await assert.rejects(() => stalled.request('/resolve'), (error: unknown) => error instanceof KclClientError && error.code === 'TIMEOUT');
  let headersCalled = false;
  const hangingHeaders = new KclClient({ baseUrl: 'https://knowledge.example', workspaceId: 'workspace-main', timeoutMs: 10, headers: async () => { headersCalled = true; return await new Promise<Record<string, string>>(() => undefined); }, fetch: async () => { throw new Error('must not fetch'); } });
  await assert.rejects(() => hangingHeaders.request('/resolve'), (error: unknown) => error instanceof KclClientError && error.code === 'TIMEOUT');
  assert.equal(headersCalled, true);
  const preaborted = new AbortController(); preaborted.abort(); let networkCalls = 0;
  const abortedClient = new KclClient({ baseUrl: 'https://knowledge.example', workspaceId: 'workspace-main', headers: () => { throw new Error('must not prepare headers'); }, fetch: async () => { networkCalls++; return new Response('{}'); } });
  await assert.rejects(() => abortedClient.request('/resolve', { signal: preaborted.signal }), (error: unknown) => error instanceof KclClientError && error.code === 'ABORTED');
...
test('validated resolve fetches and verifies the full revision, manifest slot and digest', async () => {
  ...
  const calls: string[] = [];
  const client = new KclClient({ baseUrl: 'https://knowledge.example', workspaceId: 'workspace-main', fetch: async (input) => {
...
test('validated resolve rejects development modes by default and catches revision tampering', async () => {
  ...
  await assert.rejects(() => client.resolve({ document_ids: [current.payload.document_id], context_id: current.payload.context_id, scope_id: current.payload.scope_id, usage_scope: current.payload.usage_scope }, { allowDevelopment: true }), (error: unknown) => error instanceof KclClientError && error.code === 'INVALID_CLIENT_DATA');
});

test('client validates the real demo resolve and revalidate manifest lifecycle with caller cookie and CSRF', async t => {
  const directory = mkdtempSync(join(tmpdir(), 'kcl-client-integration-'));
  ...
  const client = new KclClient({ baseUrl: origin, workspaceId: 'demo', headers: () => ({ Cookie: cookie, Origin: origin, 'X-KCL-CSRF': session.csrf_token }) });
...
test('revalidate does not accept a bare valid status without a refreshed manifest', async () => {
  const client = new KclClient({ baseUrl: 'https://knowledge.example', workspaceId: 'workspace-main', fetch: async () => new Response(JSON.stringify({ status: 'valid' })) });
  await assert.rejects(() => client.revalidate('run-client-test-001'), (error: unknown) => error instanceof KclClientError && error.code === 'INVALID_CLIENT_DATA');
});
...
test('expired header preparation cannot start a late request and malformed refreshed proof is rejected',async()=>{
  ...
  const client=new KclClient({baseUrl:'https://knowledge.example',workspaceId:'workspace-main',timeoutMs:5,headers:()=>new Promise(resolve=>release=resolve),fetch:async()=>{calls++;return new Response('{}');}});
  ...
  for(const mutate of [(m:any)=>{m.checkpoint.transaction_id='invalid';},(m:any)=>{m.private_sources=[{private_uri:'PRIVATE_URI'}];},(m:any)=>{m.approval_decisions=[];},(m:any)=>{m.checkpoint.private_debug='do-not-forward';}]){
    ...
    const broken=new KclClient({baseUrl:'https://knowledge.example',workspaceId:'workspace-main',fetch:async()=>new Response(JSON.stringify({status:'valid',refreshed_manifest:invalid,checkpoint:invalid.checkpoint}))});
    await assert.rejects(broken.revalidate('run-client-test-001'),KclClientError);
  }
});
