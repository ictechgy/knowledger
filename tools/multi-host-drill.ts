...
import { closeSync, existsSync, fchmodSync, lstatSync, mkdirSync, mkdtempSync, openSync, readdirSync, rmSync, statSync, writeFileSync } from 'node:fs';
...
export interface MultiHostDrillResult {
  ...
  details: {
    host_a: { pid: number | null; kill_signal: 'SIGKILL'; wal_sidecar_bytes: number; commits_acknowledged: number; recovered_checkpoint: string; recovered_journal_events: number; recovered_journal_digest: string };
...
interface ReadyWorker { child: ChildProcess; stderrTail: () => string; ready: WorkerReadiness; commits: string[] }
...
/** 독립 데이터 디렉터리로 워커 프로세스를 띄워 준비 신호까지 기다린다 — stderr는 실패 진단용으로 꼬리를 남기고, 이후 stdout 라인의 커밋 승인은 commits에 모은다. */
async function startWorker(dataDir: string, extraArgs: string[] = []): Promise<ReadyWorker> {
  ...
  child.stdout!.on('data', (chunk: string) => {
    ...
    while ((lineEnd = buffer.indexOf('\n')) >= 0) {
      ...
      let message: { ready?: boolean; commit?: unknown };
  ...
  try {
    ...
    return { child, ready, stderrTail: () => stderr, commits };
  } catch (error) {
...
async function readRecoveredState(dataDir: string, label: string): Promise<RecoveredState> {
  ...
  try {
    ...
    const drafts = await service.listDrafts(actorIdentity(PERSONAS[1]), 50, undefined);
    return {
...
export async function runMultiHostDrill(rootDir: string): Promise<MultiHostDrillResult> {
  ...
  try {
    ...
    // At least one loop commit must be acknowledged before the kill — an
    // acknowledged commit proves the write workload was active, not just armed.
    const commitDeadline = Date.now() + 15_000;
    while (a.commits.length === 0) {
      if (a.child.exitCode !== null || a.child.signalCode !== null) throw new Error(`write-loop domain exited before its first commit: ${a.stderrTail().slice(-200)}`);
      if (Date.now() > commitDeadline) throw new Error('write-loop domain acknowledged no commits before kill');
    ...
    result = {
      ...
      assessment: { drill_pass: true },
      details: {
        host_a: { pid: a.child.pid ?? null, kill_signal: 'SIGKILL', wal_sidecar_bytes: walBytes, commits_acknowledged: acknowledged.length, recovered_checkpoint: walState.checkpoint_tx_id, recovered_journal_events: walState.event_count, recovered_journal_digest: walState.journal_digest },
