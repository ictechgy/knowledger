...
export const DEVELOPMENT_ORGANIZATIONS = [
  { org_id: 'SalesMSP', key_id: 'person-sales-owner', subject: 'dev-sales-owner', domain: 'sales', peer_port: 17051 },
  { org_id: 'FulfillmentMSP', key_id: 'person-fulfillment-owner', subject: 'dev-fulfillment-owner', domain: 'fulfillment', peer_port: 18051 },
  { org_id: 'SettlementMSP', key_id: 'person-settlement-owner', subject: 'dev-settlement-owner', domain: 'settlement', peer_port: 19051 },
] as const;
...
export function getDevelopmentOrganization(value: unknown): DevelopmentOrganization {
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    if (keys.join('\0') !== ['domain', 'key_id', 'org_id', 'peer_port', 'subject'].join('\0')
      || record.domain !== organization.domain || record.key_id !== organization.key_id
      || record.org_id !== organization.org_id || record.peer_port !== organization.peer_port
